...
import { approveWithdrawal, rejectWithdrawal } from "@/actions/withdrawalActions";
...
export function WithdrawalActions({ transactionId, adminId }: WithdrawalActionsProps) {
    ...
    const handleApprove = async () => {
        if (!confirm('Are you sure you want to approve this withdrawal? The amount will be deducted from the customer account.')) {
            return;
        }

        setIsProcessing(true);
        const result = await approveWithdrawal(transactionId, adminId);
    ...
    const handleReject = async () => {
        setIsProcessing(true);
        const result = await rejectWithdrawal(transactionId, adminId, rejectReason);
