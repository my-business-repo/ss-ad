...
import { createOrder } from "@/actions/order-plan";
...
export function NewOrderModal({ isOpen, onClose, planId }: NewOrderModalProps) {
    ...
    // Initial load of products when modal opens
    useEffect(() => {
        if (isOpen) {
            setSearch("");
    ...
    const handleCreate = async () => {
        ...
        setSubmitting(true);
        try {
            const res = await createOrder(planId, selectedProduct.id);
            if (res.success) {
                onClose();
            } else {
                alert(res.error);
